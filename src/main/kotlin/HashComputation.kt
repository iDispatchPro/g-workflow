...
fun computeAndSaveFileHash(file : File, shaAlgorithms : List<String>) =
    shaAlgorithms
        .map { algorithm ->
            val shaSum = HashUtils.getCheckSumFromFile(MessageDigest.getInstance(algorithm),
                                                       file)

            val shaFile = File("${file.name}.${algorithm.replace("-", "").lowercase(Locale.getDefault())}")
...
private fun writeContentToFile(file : File, content : String) {
    file.bufferedWriter(UTF_8)
        .use { writer ->
